...
class UserLogin(APITestCase):
    ...
    def test_login_using_valid_credentials(self):
        client = APIClient()
        response = client.post(self.url, {'username':self.user.username, 'password':self.password})
    ...
    def test_login_using_invalid_credentials(self):
        response = self.client.post(self.url, {'username':self.user.username, 'password':'Invalid password'})
